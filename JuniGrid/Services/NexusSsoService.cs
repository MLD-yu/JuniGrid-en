...
/// Flow: connect the WebSocket → send {id:uuid} → the system browser opens the authorization page →
/// receive connection_token and send it back → receive api_key to finish.
/// v0.68.4: implemented the official protocol's "WebSocket ping every 30s" keep-alive —
/// the official spec requires continuous pinging from connect to close; otherwise, if the user lingers
/// a bit too long on the authorization page, the server deems the connection idle and drops it
/// (symptom: the user completes authorization but login still fails).
/// No hand-written ping loop is needed on the .NET side: KeepAliveInterval makes the underlying layer send ping frames automatically.
/// </summary>
public class NexusSsoService
{
    ...
    public async Task<string?> LoginAsync(string applicationSlug, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // v0.68.4: the official SSO protocol strictly requires a ping every 30s as keep-alive.
            // KeepAliveInterval makes the .NET layer send WebSocket ping frames automatically at that interval,
            // equivalent to the ws.ping() timer in the official Node example (effective on .NET 6+).
            ...
            {
                // Non-JSON (possibly a heartbeat etc.) — ignore and keep reading; logging every failure would be too noisy
                AppLog.Warn("NSSO", "Unparseable message received: " + msg[..Math.Min(msg.Length, 120)]);
            }
            }
        }
